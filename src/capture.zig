// -----------------------------------------------------------------------------
// src/capture.zig
// Clean Zig capture API. Apps use this. No C types leak out.
// Backend: Npcap (Windows) via backend/npcap_raw.zig
// -----------------------------------------------------------------------------
...
pub const CaptureHandle = struct {
    ...
    /// Pass count = -1 to loop forever; count = N to capture N packets.
    pub fn loop(
        self: *CaptureHandle,
        count: i32,
...
/// List all available network interfaces. Caller must call freeDevices() when done.
...
/// Open a saved .pcap file for offline reading.
pub fn openFile(path: [:0]const u8) CaptureError!CaptureHandle {
    clearLastError();
    var errbuf: [raw.PCAP_ERRBUF_SIZE]u8 = [_]u8{0} ** raw.PCAP_ERRBUF_SIZE;

    const handle = raw.pcap_open_offline(path.ptr, &errbuf) orelse {
        setLastErrorFromErrbuf(errbuf[0..]);
        return error.OpenFailed;
    };
